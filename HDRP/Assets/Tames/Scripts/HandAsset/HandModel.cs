...
namespace HandAsset
{
    public class HandModel
    {
        ...
        public static HandModel Duplicate(HandModel h)
        {
            GameObject control = h.data.gameObject;
            GameObject model = h.wrist;
            int index = h.handIndex;
            HandModel hdupl = new HandModel(control, model, index);
            return hdupl;
        }
        private void Init()
        {
        ...
        public void Update(Person person = null)
        {
            if (person != null)
            {
                ...
                wrist.transform.rotation = person.rotation[handIndex];
                lastTipPosition[Index] = tipPosition[Index];
                float grip = person.keyMap.vrMap.Held(handIndex == 0 ? Records.VRMap.GL : Records.VRMap.GR) ? 1 : 0;
                Grip(Index_ID, grip);
                Grip(Pinky_ID + Ring_ID + Middle_ID, grip);
                Grip(Thumb_ID, 0);
                AfterGrip(false);
                tipPosition[Index] = person.hand[handIndex].tipPosition[Index];
                lastTipPosition[Middle] = person.hand[handIndex].lastTipPosition[Middle];
                tipPosition[Middle] = person.hand[handIndex].tipPosition[Middle];
            }
            else
            {
