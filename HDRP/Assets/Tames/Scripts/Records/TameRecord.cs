...
namespace Records
{
    ...
    public class TameFrameRecord
    {
        ...
        public static FrameShot Read(BinaryReader bin, int kc)
        {
            FrameShot record = new FrameShot();
            return record;
        }
        public void Unpress()
        {
    ...
    public class TameFullRecord
    {
        ...
        public List<FrameShot> frame = new List<FrameShot>();
        public TameFullRecord(List<Multi.Person> persons)
        {
            //    this.tes = tes;
        ...
        {
            FrameShot fr = new FrameShot()
            {
                time = time,
            };
            if (km == null)
            {
                fr = TameInputControl.keyMap.ToFrameShot();
                fr.cpos = CoreTame.localPerson.headPosition;
                fr.crot = CoreTame.localPerson.headRotation;
                for (int i = 0; i < 2; i++)
                { fr.hpos[i] = CoreTame.localPerson.position[i];
                    fr.hrot[i] = CoreTame.localPerson.rotation[i];
                } }
            else
            {
                fr = km.ToFrameShot();
        ...
        private bool IsChanged(TameFrameRecord a, TameFrameRecord b, ExportOption eo)
        {
            ...
            for (int h = 0; h < (eo.bothHands ? 2 : 1); h++)
            {
                ...
                //          if (eo.actionMouse)                    if (a.keyMap.mouse.U != b.keyMap.mouse.U) return true;
                if (eo.actionGamePad)
        ...
        public void ExportToCSV(string folder, ExportOption eo)
        {
            ...
            for (int j = 0; j < allRecords.frame.Count; j++)
            {
              ...
              //  p = f.person;
                ...
                else changed = j == 0 ? true : allRecords.frame[j].Changed(allRecords.frame[j - 1], eo);
                ...
                if (changed)
                {
                    ...
                    //       if (eo.actionMouse)                        s += f.keyMap.mouse.Export(f.keyMap.mouse.pressed) + "," + f.keyMap.mouse.Export(f.keyMap.mouse.hold) + ",";
                    //      if (eo.actionGamePad)
