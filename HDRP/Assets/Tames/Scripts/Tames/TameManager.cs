       ...
       // public List<ManifestBase> manifests;
        ...
        public void Initialize(string[] l = null)
        {
            ...
            foreach (GameObject rootObj in RootObjects)
                mis.AddRange(rootObj.GetComponentsInChildren<Markers.MarkerInfo>(true));
        ...
        public static string[] EmailBody()
        {
            ...
            foreach (InfoUI.InfoControl fo in info)
                foreach (InfoUI.InfoFrame fi in fo.frames)
                    if (fi.choice.Count > 0)
                        for (int i = 0; i < fi.choice.Count; i++)
                            list.Add("Choice," + fo.marker.name + "," + fi.index + "," + i + "," + fi.choice[i].text + "," + fi.choice[i].selected);
            foreach (TameAlternative ta in altering)
                list.Add("Alter," + ta.marker.name + "," + ta.current);
       //     Debug.Log(alteringMaterial == null);
            foreach (TameMaterialAlternative ta in alteringMaterial)
            {
        //        Debug.Log(ta.marker.name);
                list.Add("Material," + ta.marker.name + "," + ta.current);
            }
            foreach (TameScoreBasket ba in basket)
                list.Add("Basket," + ba.name + "," + ba.totalScore);
            return list.ToArray();
        }
        public static TameThing FindThing(GameObject g)
        {
            ...
            Markers.MarkerAlterMaterial mam;
            Markers.MarkerAlterObject mao;
            Markers.MarkerScore ms;
            ...
            if (tgo != null)
            {
                ...
                else if ((mam = tgo.gameObject.GetComponent<Markers.MarkerAlterMaterial>()) != null)
                {
                    TameMaterialAlternative tma = alteringMaterial.Find(x => x.marker = mam);
                    if (tma != null)
                    {
                        r = tma;
                    }
                }
                else if ((mao = tgo.gameObject.GetComponent<Markers.MarkerAlterObject>()) != null)
                {
                    TameAlternative ta = altering.Find(x => x.marker = mao);
                    if (ta != null)
                    {
                        r = ta;
                    }
