...
namespace Multi
{
    public class ITameEffect
    {
        ...
        public void Set(Updater te)
        {
       //     tameIndex = te.target.index;
            parent = (te.sourceType == TrackBasis.Object) || (te.sourceType == TrackBasis.Head) || (te.sourceType == TrackBasis.Hand) ? Position : Progress;
            if (parent == Progress)
            {
       //         if (te.target.progress != null) progress = te.target.progress.progress;
            }
            else
