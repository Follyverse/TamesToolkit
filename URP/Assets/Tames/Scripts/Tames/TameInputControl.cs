...
namespace Tames
{
    ...
    public class TameInputControl
    {
        ...
        public static TameKeyMap keyMap;
        ...
        public bool mono;
        public int[] keyValue;
        public static int hooverDirection;
        public static bool choicePressed;
        public static void ChoiceHoover()
        {
            hooverDirection = 0;
            choicePressed = false;
            if (Keyboard.current != null)
                ...
                {
                    if (Keyboard.current.leftArrowKey.wasPressedThisFrame) hooverDirection = -1;
                    if (Keyboard.current.rightArrowKey.wasPressedThisFrame) hooverDirection = 1;

                    if (Keyboard.current.enterKey.wasPressedThisFrame) choicePressed = true;
                }
            if (Gamepad.current != null)
            {
                if (keyMap.gpMap.Held(GPMap.TL) && keyMap.gpMap.Pressed(GPMap.SLLEFT)) hooverDirection = -1;
                if (keyMap.gpMap.Held(GPMap.TL) && keyMap.gpMap.Pressed(GPMap.SLRIGHT)) hooverDirection = 1;
            }

            if (keyMap.vrMap.Held(VRMap.TL) && keyMap.vrMap.Pressed(VRMap.SLLEFT)) hooverDirection = -1;
            if (keyMap.vrMap.Held(VRMap.TL) && keyMap.vrMap.Pressed(VRMap.SLRIGHT)) hooverDirection = 1;
        }
        public static TameKeyMap CheckKeys(int index = -1)
        {
            if (index >= 0)
                keyMap = null;// Records.TameFullRecord.allRecords.frame[index].keyMap;
            else
            {
                keyMap.Capture();
                return keyMap;
        ...
        public static List<TameInputControl> AllMonos(Markers.InputSetting keys)
        {
            ...
            string[] key = keys.key.Split(' ');
            TameInputControl tci;
            foreach (string s in key)
            {
                if (keys.key != "")
                {
                    tci = TameInputControl.StringToMono(s, InputTypes.KeyboardMouse, keys.Aux(0));
                ...
                default:
                    {
                        ...
                        if (hold == InputHoldType.Button)
                            return keyMap.hold[TameKeyMap.RightMouse] ? 1 : (keyMap.hold[TameKeyMap.LeftMouse] ? -1 : 0);
                ...
                case InputTypes.VRController:
                    switch (hold)
                    {
                        ...
                        case InputHoldType.VRScrollLeft:
                            return keyMap.vrMap.Pressed(VRMap.THLDOWN + VRMap.THLUP);
                        //     if (Mathf.Abs(f) > Threshold) return true;
                        //        break;
                        case InputHoldType.VRScrollRight:
                            return keyMap.vrMap.Pressed(VRMap.THRDOWN + VRMap.THRUP);
                        //  f = keyMap.vrMap.thumb[1];
                        //     if (Mathf.Abs(f) > Threshold) return true;
                        //      break;
                        case InputHoldType.VRTrigger:
                            return keyMap.vrMap.Pressed(VRMap.TL + VRMap.TR);
                ...
                default:
                    {
                        ...
                        else
                        {
                            ...
                            if (keyValue == null) return keyMap.pressed[TameKeyMap.LeftMouse] || keyMap.pressed[TameKeyMap.RightMouse];
                            else if (keyValue[0] == -1) return keyMap.pressed[TameKeyMap.LeftMouse];
                            else return keyMap.pressed[TameKeyMap.RightMouse];
                        }
                    }
            }
        }
