...
namespace Tames
{
    ...
    public class TameProgress
    {
        ...
        public void SetByTime(float deltaTime)
        {
            ...
            //  if (element.name == "item1") Debug.Log(isMultiAlter);
        ...
        void SetJustCycled()
        {
            ...
            {
                case ContinuityMode.Stop:
                    if (progress > 0 && lastProgress == 0) justCycled = 1;
                    if (progress < 1 && lastProgress == 1) justCycled = -1;
                    break;
                case ContinuityMode.Reverse:
                    if ((int)totalProgress != (int)lastTotal)
                    {
                        if (totalProgress % 2 == 1 && lastTotal % 2 == 0) justCycled = -1;
                        if (totalProgress % 2 == 0 && lastTotal % 2 == 1) justCycled = 1;
                    }
                    break;
                case ContinuityMode.Cycle:
                    if ((int)totalProgress != (int)lastTotal)
       ...
       //     bool cycled = false;
        ...
        public void AddStatus(RiptideNetworking.Message m)
        {
            ...
            {
                m.AddByte(1);
                m.AddFloat(totalProgress);
                m.AddFloat(progress);
                m.AddFloat(subProgress);
                m.AddInt(fromAlter);
                m.AddInt(toAlter);
                m.AddInt(initiated);
            }
        ...
        public void GetStatus(RiptideNetworking.Message m)
        {
            ...
            switch (header)
            {
                case 0: return;
                case 1:
                    totalProgress = m.GetFloat();
                    progress = m.GetFloat();
                    subProgress = m.GetFloat();
                    fromAlter = m.GetInt();
                    toAlter = m.GetInt();
                    initiated = m.GetInt();
                    Debug.Log("alter "+fromAlter + " " + toAlter);
                    break;
                case 2:
                    totalProgress = m.GetFloat();
                    progress = m.GetFloat();
